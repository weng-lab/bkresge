...
cell_type_col_name <- "superfine.cell.class"
...
dataset <- "HPC"
load_and_rename(hpc_spe_path, "spe", verbose = TRUE)
load_and_rename(hpc_sce_path, "sce", verbose = TRUE)
...
ecdf_plot_file <- here("plots", "ecdf_nonzero_nuclei_spots_nmf_patterns_faceted.pdf")
...
ggsave(
    filename = here("plots", "dotplot_entropy_bimodal_split.pdf"),
    plot = split_plot,
    height = 6, width = 8
)
...
ggsave(
    filename = here("plots", "dotplot_nmf_patterns_by_cell_type.pdf"),
    plot = p1,
    height = 8,
    width = 16
)
...
# Save spot dotplot
ggsave(
    filename = here("plots", "dotplot_nmf_patterns_by_srt_domain.pdf"),
    plot = p2,
    height = 6,
    width = 12
)
